...
import SignupPage from './pages/SignupPage'; // develop 브랜치에서 추가된 SignupPage 임포트
...
function App() {
  ...
  // 새로운 상태: 웰컴 화면을 이미 봤는지 여부 (develop 브랜치에서 통합)
  ...
  // 컴포넌트 마운트 시 localStorage에서 로그인 및 웰컴 화면 확인 상태를 확인 (develop 브랜치에서 통합)
  useEffect(() => {
    // 로그인 상태 확인
  ...
  const handleLogin = () => {
    setIsLoggedIn(true);
    localStorage.setItem('isLoggedIn', 'true'); // 로그인 상태 유지 로직 유지 (develop 브랜치에서 통합)
  };

  // 새로운 함수: 웰컴 화면을 봤다고 표시하는 함수 (develop 브랜치에서 통합)
  ...
  return (
    <div className="App">
      <Routes>
        {/* 앱의 초기 진입점: 웰컴 화면을 봤는지 여부에 따라 리다이렉트 (develop 브랜치에서 통합) */}
        <Route 
          path="/" 
        ...
        <Route path="/signup" element={<SignupPage />} /> {/* develop 브랜치에서 추가된 회원가입 라우트 */}
