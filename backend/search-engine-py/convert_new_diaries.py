...
def convert_new_diaries():
    """새로운 일기들을 날짜 정보를 포함하여 벡터 임베딩으로 변환"""
    try:
        # MySQL 연결
        ...
        # 새로운 일기들 조회 (ID 17-23)
        cursor.execute('SELECT id, user_id, title, content, mood, created_at FROM parent_diaries WHERE id >= 17 ORDER BY id')
        diaries = cursor.fetchall()
        
        if not diaries:
            return {"success": False, "message": "변환할 일기가 없습니다."}
        
        print(f"총 {len(diaries)}개의 새로운 일기를 벡터 임베딩으로 변환합니다...")
        ...
        for diary in diaries:
            try:
                ...
                if '8월' in title and '일' in title:
                    date_str = title.split(' ')[0] + ' ' + title.split(' ')[1]  # "8월 11일"
                else:
                    date_str = str(diary['created_at'])[:10]  # "2025-08-11"
                ...
                vector_cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?)
                ''', (diary['id'], text, json.dumps(embedding), date_str))
                
                converted_count += 1
                print(f"일기 ID {diary['id']}: 변환 완료")
                print(f"  - 텍스트: {text[:50]}...")
...
def main():
    """메인 함수"""
    try:
        print("새로운 일기들을 날짜 정보를 포함하여 벡터 임베딩으로 변환을 시작합니다...")
        result = convert_new_diaries()
